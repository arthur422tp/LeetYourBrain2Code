...
`;
    ...
    if (result.ok) {
      ...
      expect(result.request.limits.maxExpressionEvents).toBe(20_000);
      ...
      expect(result.request.limits.maxCallFrameEvents).toBe(20_000);
      expect(result.request.limits.maxCallFrameBytes).toBe(2_000_000);
    }
  });
